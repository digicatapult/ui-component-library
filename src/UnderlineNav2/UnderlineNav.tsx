...
const overflowEffect = (
  ...
  const numberOfItemsPossible = calculatePossibleItems(
    childWidthArray,
    navWidth
  )
  ...
  if (childArray.length <= numberOfItemsPossible) {
    ...
    const numberOfItemsInMenu =
      childArray.length - numberOfItemsPossibleWithMoreMenu
...
export const UnderlineNav = forwardRef(
  (
    ...
    function getItemsWidth(itemText: string): number {
      return (
        noIconChildWidthArray.find((item) => item.text === itemText)?.width || 0
      )
    }
    ...
    const swapMenuItemWithListItem = (
      ...
      const widthToFitIntoList = getItemsWidth(
        prospectiveListItem.props.children
      )
      ...
      const index = getBreakpointForItemSwapping(
        widthToFitIntoList,
        availableSpace
      )
      const indexToSliceAt = responsiveProps.items.length - 1 - index
      // Form the new list of items
      const itemsLeftInList = [...responsiveProps.items].slice(
        0,
        indexToSliceAt
      )
      ...
      updatedMenuItems.splice(
        indexOfProspectiveListItem,
        1,
        ...itemsToAddToMenu
      )
    ...
    function getBreakpointForItemSwapping(
      widthToFitIntoList: number,
      availableSpace: number
    ) {
      let widthToSwap = 0
      let breakpoint = 0
      for (const [index, item] of [...responsiveProps.items]
        .reverse()
        .entries()) {
    ...
    const [noIconChildWidthArray, setNoIconChildWidthArray] =
      useState<ChildWidthArray>([])
    ...
    return (
      ...
      >
        {ariaLabel && (
          <VisuallyHidden as="h2">{`${ariaLabel} navigation`}</VisuallyHidden>
        )}
        <Box
          as={as}
          sx={merge<BetterSystemStyleObject>(
            getNavStyles(theme, { align }),
            sxProp
          )}
          aria-label={ariaLabel}
          ref={navRef}
