    ...
    useLayoutEffect(() => {
      if (ref.current) {
        ...
        const icon = Array.from(
          (ref as MutableRefObject<HTMLElement>).current.children[0].children
        ).find((child) => child.getAttribute('data-component') === 'icon')
        ...
        if (selectedLinkText === text) {
          setSelectedLink(ref as RefObject<HTMLElement>)
          if (typeof onSelect === 'function' && selectEvent !== null)
      ...
      (event) => {
        if (event.key === ' ' || event.key === 'Enter') {
          if (!event.defaultPrevented && typeof onSelect === 'function')
            onSelect(event)
          if (!event.defaultPrevented && typeof afterSelect === 'function')
            afterSelect(event)
          setSelectedLink(ref as RefObject<HTMLElement>)
        }
      },
    ...
    return (
      ...
      >
        <Box
          ...
          aria-current={ariaCurrent}
          sx={merge(
            getLinkStyles(theme, { variant }, selectedLink, ref),
            sxProp as SxProp
          )}
          {...props}
          ref={ref}
