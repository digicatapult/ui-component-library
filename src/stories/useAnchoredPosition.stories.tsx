...
export const UseAnchoredPosition = (args: any) => {
  const { floatingElementRef, anchorElementRef, position } =
    useAnchoredPosition(
      {
        side: `${args.anchorPosition ?? 'outside'}-${
          args.anchorSide ?? 'bottom'
        }` as AnchorSide,
        align: args.anchorAlignment ?? 'start',
        anchorOffset: args.anchorOffset && parseInt(args.anchorOffset, 10),
        alignmentOffset:
          args.alignmentOffset && parseInt(args.alignmentOffset, 10),
...
export const ComplexAncestry = () => {
  ...
  return (
    <>
      ...
      >
        Clipping container - this element has <code>overflow</code> set to
        ...
        >
          Relatively positioned parent, but fluid height, so not the clipping
          parent.
          <Box
            m={space}
            p={space}
          ...
          >
            Floating element container. Position=static and overflow=hidden to
            show that overflow-hidden on a statically-positioned element will
            not have any effect.
            <Float
              top={position?.top ?? 0}
              left={position?.left ?? 0}
        ...
        <Box
          ...
          p={space}
          backgroundColor="blue.3"
          sx={{ border: '1px solid #000', height: '2000px' }}
        >
          Anchor element container. This element is really tall to demonstrate
          behavior within a scrollable clipping container.
          <Box
            width="200px"
  ...
  return (
    <Main ref={mainRef}>
      <Nav>
        <h2>The nav bar!</h2>
        <p>
          This &ldquo;nav bar&rdquo; has a width of 300px and is{' '}
          <code>position:relative</code> with <code>overflow:hidden</code>,
          meaning that its children cannot overflow this container. Using
          &lt;Portal&gt; with <code>useAnchoredPosition</code>, we can break out
          of this constraint.
        </p>
