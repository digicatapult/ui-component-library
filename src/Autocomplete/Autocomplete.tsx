...
const reducer = (state: State, action: Action) => {
  ...
  switch (type) {
    ...
    case 'isMenuDirectlyActivated':
      return {
        ...state,
        isMenuDirectlyActivated: payload as State['isMenuDirectlyActivated'],
      }
    case 'autocompleteSuggestion':
      return {
        ...state,
        autocompleteSuggestion: payload as State['autocompleteSuggestion'],
      }
    case 'selectedItemLength':
      return {
        ...state,
        selectedItemLength: payload as State['selectedItemLength'],
      }
    default:
      return state
  }
...
const Autocomplete: React.FC<React.PropsWithChildren<{ id?: string }>> = ({
  children,
  id: idProp,
}) => {
  ...
  const setAutocompleteSuggestion = useCallback(
    (value: State['autocompleteSuggestion']) => {
      dispatch({ type: 'autocompleteSuggestion', payload: value })
    },
    []
  )
  const setIsMenuDirectlyActivated = useCallback(
    (value: State['isMenuDirectlyActivated']) => {
      dispatch({ type: 'isMenuDirectlyActivated', payload: value })
    },
    []
  )
  const setSelectedItemLength = useCallback(
    (value: State['selectedItemLength']) => {
